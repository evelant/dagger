...
func (fe *frontendPlainLLM) Run(ctx context.Context, opts dagui.FrontendOpts, run func(context.Context) (cleanups.CleanupF, error)) error {
	...
	if !opts.Silent {
		fmt.Fprintln(fe.out, "dagger: started")
...
func (fe *frontendPlainLLM) renderCompletedLocked(final bool) {
	for _, span := range fe.db.Spans.Order {
		...
		if !fe.shouldAnnounceSpanLocked(span, final) {
			continue
		}
		fmt.Fprintln(fe.out, fe.spanCompletionLineLocked(span))
		fe.announced[span.ID] = struct{}{}
	}
}

func (fe *frontendPlainLLM) shouldAnnounceSpanLocked(span *dagui.Span, final bool) bool {
	...
	return !span.IsCached() && span.Activity.Duration(time.Now()) >= plainLLMLongStep && fe.isTopLevelSpanLocked(span)
}

func (fe *frontendPlainLLM) isTopLevelSpanLocked(span *dagui.Span) bool {
	return !span.ParentID.IsValid() || span.ParentID == fe.db.PrimarySpan
}
...
func (fe *frontendPlainLLM) renderStatusLocked(force bool) {
	now := time.Now()
	if !force && now.Sub(fe.lastStatus) < plainLLMStatusInterval {
		return
	}
	...
	elapsed := dagui.FormatDuration(now.Sub(fe.started))
	parts := []string{
		"progress",
		"elapsed=" + elapsed,
	...
	fmt.Fprintln(fe.out, strings.Join(parts, " "))
	fe.lastStatus = now
}
...
func (fe *frontendPlainLLM) renderSummaryLocked(reportErr error) {
	...
	parts := []string{
		"dagger:",
		"result=" + status,
		"elapsed=" + dagui.FormatDuration(time.Since(fe.started)),
		fmt.Sprintf("done=%d", stats.done()),
		fmt.Sprintf("cached=%d", stats.cached),
		fmt.Sprintf("failed=%d", stats.failed),
	}
...
func (fe *frontendPlainLLM) progressStatsLocked(now time.Time) plainProgressStats {
	...
	for _, span := range fe.db.Spans.Order {
		if !fe.shouldCountSpanLocked(span) {
			continue
		}
		...
		case span.IsRunningOrEffectsRunning():
			stats.running++
			if len(stats.active) < 3 {
				label := truncateProgress(fe.spanLabelLocked(span), plainLLMMaxLabelLen)
				if dur := span.Activity.Duration(now); dur > 0 {
					label += " " + dagui.FormatDuration(dur)
				}
				stats.active = append(stats.active, label)
			}
		case span.IsPending():
			stats.pending++
		case span.IsCanceled():
...
func (fe *frontendPlainLLM) shouldCountSpanLocked(span *dagui.Span) bool {
	if span == nil || !span.Received || span.ID == fe.db.PrimarySpan {
		return false
	}
	if span.CheckName != "" || span.IsFailedOrCausedFailure() {
		return true
	}
	return fe.opts.ShouldShow(fe.db, span)
}
